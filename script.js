...
const optionalBlock = document.getElementById('optional-block');
...
/* On a narrow screen the results sit below the fold, so bring them up.
   On desktop the two panels are side by side and scrolling would be noise. */
function revealResults() {
  if (!window.matchMedia('(max-width: 899px)').matches) return;

  const smooth = !window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const target = results.closest('.panel') || results;
...
/* HTMLFormElement exposes its controls as named properties, so a control
   whose id collides with a method shadows it. Calling the prototype method
   keeps Clear working no matter what the markup is named. */
...
/* The optional block sits dimmed until it holds a value, so it never reads
   as a required step waiting to be filled. */
function syncOptionalState() {
  const inUse = systolicIn.value.trim() !== '' || diastolicIn.value.trim() !== '';
  optionalBlock.classList.toggle('is-engaged', inUse);
}

[systolicIn, diastolicIn].forEach(input => {
  input.addEventListener('input', syncOptionalState);
});
